...
const reqListner = (req, res) => {
    ...
    if (url === "/") {
        res.write(
            '<html><head><title>Hello world</title></head><body><form action="/message" method="POST"><input type="text" placeholder="Ente your message" name="message"/><button type="submit">Send</button></form></body></html>'
        );
        return res.end();
    }
